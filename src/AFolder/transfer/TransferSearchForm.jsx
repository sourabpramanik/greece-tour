...
const TransferSearchForm = ({haveDefaultValue, btnType, sendToParent}) => {
    ...
    const totalGuests = (haveDefaultValue && haveDefaultValue?.guests) ? haveDefaultValue?.guests : 1;
    const [guestValue, setGuestValue] = useState(totalGuests);
    ...
    useEffect(() => {
        if (btnType === "filter" && haveDefaultValue) {
            ...
            const tada = searchPlaceholder.reduce(
                (result, item) =>
                    haveDefaultValue?.locations.some((el) => el.Locations === item.placeId)
                        ? [...result, item]
                        : result,
                []
            );
    ...
    const checkButton = () => {
        ...
        if (haveDefaultValue == null || btnType == null) {
            return <ButtonSubmit sendDataToParent={(e) => click(e)}
            ...
            return <ButtonSubmit label={"Update Route"} origin={pickUpInputValue}
                                  destination={dropOffInputValue}
                                  date={dateValue}
                                  guests={guestValue}
    ...
    const renderForm = () => {
        return (
            <form
                className="w-full relative mt-4 md:mt-8 flex flex-col md:flex-row md:items-center rounded-3xl md:rounded-full shadow-xl dark:shadow-2xl bg-white dark:bg-neutral-900 divide-y divide-neutral-200 dark:divide-neutral-700  md:divide-y-0">
                <LocationInput
                    placeHolder="Pick-Up"
                ...
                <div className="px-4 py-4 lg:py-0 mr-6">
                    {checkButton()}
                </div>
            </form>
        );
